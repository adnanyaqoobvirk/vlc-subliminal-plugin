...
function activate()
  ...
  assert(package.loadlib('./lunatic_python.so', 'luaopen_python'))()

  if vlc.input.item() then
    url = require "net.url"
    video_path = url.parse(vlc.input.item():uri()).path
    ...
    subtitle_path = python.eval("os.path.splitext(videos[0].name)[0] + '.en.srt'")
